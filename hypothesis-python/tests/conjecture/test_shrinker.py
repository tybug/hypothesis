...
from tests.conjecture.common import SOME_LABEL, run_to_nodes, shrinking_from
...
def test_permits_but_ignores_raising_order(monkeypatch):
    ...
    monkeypatch.setattr(
        Shrinker, "shrink", lambda self: self.consider_new_choices((2,))
    )
...
def test_can_simultaneously_lower_non_duplicated_nearby_blocks(n_gap):
    ...
    shrinker.fixate_shrink_passes(["lower_nodes_together"])
