...
def test_too_small_to_be_useful_coin():
    data = ConjectureData.for_ir(ir(True))
...
def test_draw_string():
    data = ConjectureData.for_ir(ir("0"))
...
def test_fixed_size_draw_many():
    many = cu.many(ConjectureData.for_ir([]), min_size=3, max_size=3, average_size=3)
    assert many.more()
    assert many.more()
    assert many.more()
...
def test_astronomically_unlikely_draw_many():
    ...
    data = ConjectureData.for_ir(ir(True) * 1000)
...
def test_rejection_eventually_terminates_many():
    many = cu.many(
        ConjectureData.for_ir(ir(True) * 1000),
        min_size=0,
        max_size=1000,
        average_size=100,
    )
...
def test_many_with_min_size():
    many = cu.many(
        ConjectureData.for_ir(ir(False) * 5), min_size=2, average_size=10, max_size=1000
    )
    assert many.more()
    assert many.more()
    assert not many.more()
...
def test_many_with_max_size():
    many = cu.many(
        ConjectureData.for_ir(ir(True) * 5), min_size=0, average_size=1, max_size=2
    )
    assert many.more()
    assert many.more()
    assert not many.more()
