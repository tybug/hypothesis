...
def test_triviality():
    d = ConjectureData.for_ir((True, False, b"1"))
...
def test_can_observe_draws():
    ...
    d = ConjectureData.for_ir((True, 1, 3), observer=observer)
...
def test_calls_concluded_implicitly():
    ...
    d = ConjectureData.for_ir((True,), observer=observer)
    d.draw_boolean()
    d.freeze()
...
def test_examples_show_up_as_discarded():
    d = ConjectureData.for_ir((True, False, True))
...
def test_examples_support_negative_indexing():
    d = ConjectureData.for_ir((True, True))
    d.draw(st.booleans())
    d.draw(st.booleans())
    d.freeze()
...
def test_trivial_before_force_agrees_with_trivial_after():
    d = ConjectureData.for_ir((False, True, True))
    d.draw_boolean()
...
def test_child_indices():
    d = ConjectureData.for_ir((True,) * 4)
...
def test_example_equality():
    d = ConjectureData.for_ir((False, False))
