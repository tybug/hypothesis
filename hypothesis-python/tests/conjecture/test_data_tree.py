...
def test_can_go_from_interesting_to_valid():
    tree = DataTree()
    data = ConjectureData.for_ir([], observer=tree.new_observer())
...
def test_concluding_at_prefix_is_flaky():
    tree = DataTree()
    data = ConjectureData.for_ir((True,), observer=tree.new_observer())
    data.draw_boolean()
    with pytest.raises(StopTest):
        data.conclude_test(Status.INTERESTING)
...
def test_extending_past_conclusion_is_flaky():
    ...
    data = ConjectureData.for_ir((True, False), observer=tree.new_observer())
    data.draw_boolean()
...
def test_truncates_if_seen():
    ...
    data = ConjectureData.for_ir(nodes, observer=tree.new_observer())
    data.draw_integer()
    data.draw_integer()
    data.freeze()
...
def test_child_becomes_exhausted_after_split():
    tree = DataTree()
    data = ConjectureData.for_ir((b"\0", b"\0"), observer=tree.new_observer())
    data.draw_bytes(1, 1)
    data.draw_bytes(1, 1, forced=b"\0")
    data.freeze()

    data = ConjectureData.for_ir((b"\1", b"\0"), observer=tree.new_observer())
    data.draw_bytes(1, 1)
    data.draw_bytes(1, 1)
    data.freeze()
...
def test_will_generate_novel_prefix_to_avoid_exhausted_branches():
    tree = DataTree()
    data = ConjectureData.for_ir((1,), observer=tree.new_observer())
    data.draw_integer(0, 1)
    data.freeze()

    data = ConjectureData.for_ir((0, b"\1"), observer=tree.new_observer())
    data.draw_integer(0, 1)
    data.draw_bytes(1, 1)
    data.freeze()
...
def test_will_mark_changes_in_discard_as_flaky():
    tree = DataTree()
    data = ConjectureData.for_ir((1, 1), observer=tree.new_observer())
...
def test_is_not_flaky_on_positive_zero_and_negative_zero():
    ...
    data = ConjectureData.for_ir((0.0,), observer=tree.new_observer())
    f = data.draw_float()
...
def test_low_probabilities_are_still_explored():
    ...
    data = ConjectureData.for_ir(false_ir, observer=tree.new_observer())
...
def test_can_generate_hard_values():
    ...
    for i in range(max_value):
        data = ConjectureData.for_ir([i], observer=tree.new_observer())
    ...
    for _ in range(20):
        ...
        data = ConjectureData.for_ir(prefix)
...
def test_simulate_forced_floats(node):
    ...
    data = ConjectureData.for_ir([node.value], observer=tree.new_observer())
